...
\begin{frame}[plain]
  ...
  \textcolor{evnxMuted}{evnx 0.5.0 @ tag \texttt{v0.5.0} (\texttt{85a52c3}). Every terminal block is a
  real run.}
  \end{center}
\end{frame}
...
evnx vault share shop/production --with teammate@example.com
...
evnx auth token create ci --scope read --vault shop/production
...
\begin{frame}{Pick the workflow that fits}
  ...
  \vspace{0.6em}
  {\footnotesize \textbf{Installing with cargo?} Use
  \texttt{cargo install evnx --all-features} --- the plain form compiles with
  \texttt{default = []} and omits \texttt{cloud}, \texttt{backup} and
  \texttt{migrate}. Every other channel ships all of them.}
\end{frame}
